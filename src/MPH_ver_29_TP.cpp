...
conversion factor to convert mph to kph     mph * 1.60934 = kph
 software counts the pulses on an interupt for 500ms and then calculates speed
...
ver 1.27    Added checkboxes to option screen for radar or gps input    This version sent to JOhn Gibbs (tractor puller) to test at event
            John is using a radar sensor
...
ver 1.28    Add pulsing alarm light that flashes faster as you near target. 
            Created a 100ms timer with interupt routine to flash alarm light


(software version on line 178)
*/
//send the following code to the GPS sensor to output the RMC string:
//     $ /P /G /R /M /O / ' /G/P /R /M /C / ' /1/cr/lf
//     24/50/47/52/4D/4F/2C/47/50/52/4D/43/2C/31/0D/0A
//below is the GPRMC string we are decoding
//GPRMC,154030.4,A,3935.76900,N,08610.28625,W,000.02,240.3,171020,004.5,W*0.05C  (string copied from serial monitor

/*prog_char str3[] PROGMEM = "$GPRMC,201548.000,A,3014.5529,N,09749.5808,W,000.17,53.25,040109,E,A,*2B";  //000.17 is speed in knots (7th position)
      ...
      7= speed Kn (Speed in knots)
      8= track true ( Track made good,degrees good)
      9= date: dd/mm/yy
      10= mag var (magnetic variation)
      11= var dir (magnetic variation direction)
      12= mode ind (Positioning system mode indicator)
      13= Check Sum
      14= [CR][LF] Sentence terminiator
...
/*
   ...
   has an excellent high-sensitivity receiver (-165 dB tracking!), and a built in antenna. It can do up to 10 location updates a second for
   high speed, high sensitivity logging or tracking. Power usage is incredibly low, only 20 mA during navigation.
*/
...
#include <stdio.h>
#include "lv_ex_conf.h"             //config file to setup lines that go to display and touch screen
